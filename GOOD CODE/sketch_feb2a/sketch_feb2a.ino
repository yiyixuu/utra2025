...
const float MOTOR_POWER = 0.25;
const int ninety_time = 1500;
...
// Calibration Values
const int ref_black[] = {30, 17, 6};
const int ref_blue[] = {220, 255, 255};
const int ref_green[] = {225, 255, 240};
const int ref_red[] = {247, 244, 238};
...
const unsigned long COLOR_UPDATE_INTERVAL = 150;  // Update interval (ms)
...
int offset = 500;  // Set the final reverse time in milliseconds
...
void updateColorNonBlocking() {
    if (millis() - lastColorUpdate >= COLOR_UPDATE_INTERVAL) {
        ...
        int redValue = mapColor(redPW, 200, 920);
        int greenValue = mapColor(greenPW, 445, 1530);
        int blueValue = mapColor(bluePW, 370, 1360);
...
void turnLeftForTime(unsigned long duration) {
    if (!isTurningLeft) {
        ...
        drive("left", MOTOR_POWER);  // Start turning left
...
void loop() {
    ...
    if (isDrivingInitially) {
        ...
        reverseForTime(timer / 2);  // Reverse for half the detected time
